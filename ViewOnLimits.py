...
from threading import Thread, active_count,Event, enumerate
...
channel, post, time_out, real_views,limit = '', 0, 15, 0, 100000
threads=[]
...
def scrap(sources, _proxy_type):
    for source in sources:
        if source and not stop_event.is_set():
            try:
...
# async def view_main(chann,message,viewMaxlimit):
...
#     global real_views    
#     value=float(real_views.replace('K','')) * 1000 if 'K' in real_views else float(real_views)
#     while value < limit and not stop_event.is_set():
#         await asyncio.sleep(5)  # Sleep for a while before checking again
#         print(real_views)
#         value=float(real_views.replace('K','')) * 1000 if 'K' in real_views else float(real_views)

#     print('Stopping view_main and active threads...')
...
async def view_main(chann, message, viewMaxlimit):
    ...
    try:
        ...
        print(' [ OUTPUT ] Started | Wait a few seconds to run threads')
        Thread(target=start_view).start()
        Thread(target=check_views).start()
        sleep(7.5)
    ...
    # Set the stop_event to signal threads to stop
    stop_event.set()

    # Terminate all active threads immediately
    for thread in enumerate():
        if isinstance(thread, Thread):
            thread.terminate()

asyncio.run(view_main('ViewTestChannel12', 41, 3500))