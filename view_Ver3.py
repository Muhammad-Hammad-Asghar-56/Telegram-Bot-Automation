...
async def view_main(chann,message,limit):
    ...
    try:
        ...
        task=Thread(target=start_view).start()
        task2=Thread(target=check_views).start()
        threads.append(task)
        threads.append(task2)
        sleep(7.5)
    
    global real_views
    value = float(real_views.replace('K', '')) * 1000 if 'K' in real_views else float(real_views) # Calcualte the
    
    while value < limit and not stop_event.is_set():
        await asyncio.sleep(2)  # Sleep for a while before checking again
        print(real_views)
        value = float(real_views.replace('K', '')) * 1000 if 'K' in real_views else float(real_views)
    
    for thread in enumerate():
        if isinstance(thread, Thread):
            print("1 thread was cancel forcfully")
            thread.terminate()

asyncio.run(view_main('ViewTestChannel12',42,200))